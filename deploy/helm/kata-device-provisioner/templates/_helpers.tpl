...
{{/*
Non-empty when the NodeFeatureRule is wanted. One cluster-scoped object serves
every profile, so it is rendered outside renderProfiles and asked for by the
release or by any enabled profile.
*/}}
{{- define "kata-device-provisioner.nodeFeatureRuleEnabled" -}}
{{- if (.Values.nodeFeatureRule | default dict).enabled }}true{{ end -}}
{{- range $profile := (.Values.profiles | default dict) -}}
{{- if and $profile.enabled (($profile.nodeFeatureRule | default dict).enabled) }}true{{ end -}}
{{- end -}}
{{- end -}}
...
{{- /* Ownership is judged by the instance marker, not by --node-label-key, so
       leaving the instance at its default would make every profile claim every
       node: each would then read the others' nodes as ones that left its own
       selection, and unprovision them. */}}
- "--instance-label-prefix=kata-device-provisioner.katacontainers.io"
- "--multi-install-suffix={{ include "kata-device-provisioner.profileSlug" $root }}"
